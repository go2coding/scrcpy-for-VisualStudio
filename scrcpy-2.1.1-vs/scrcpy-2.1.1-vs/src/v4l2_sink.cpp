...
run_v4l2_sink(void *data) {
    struct sc_v4l2_sink *vs = (struct sc_v4l2_sink*)data;
...
sc_v4l2_sink_open(struct sc_v4l2_sink *vs, const AVCodecContext *ctx) {
    ...
    ostream = avformat_new_stream(vs->format_ctx, encoder);
    ...
    r = avcodec_parameters_from_context(ostream->codecpar, ctx);
    if (r < 0) {
        goto error_avformat_free_context;
    }
    ...
    ret = avio_open(&vs->format_ctx->pb, vs->device_name, AVIO_FLAG_WRITE);
    if (ret < 0) {
