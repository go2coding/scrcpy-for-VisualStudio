...
#define HEADER_LEN (sizeof(HEADER) - 1)
    ...
    while (str[idx_line] != '\0') {
        ...
        //sc_vector_reserve
        size_t mincap = (out_vec)->size + 1;
        /* avoid to allocate tiny arrays (< SC_VECTOR_MINCAP_) */ 
            size_t mincap_ = sc_vector_max_(mincap, SC_VECTOR_MINCAP_); 
            if (mincap_ <= (out_vec)->cap) {
                    /* nothing to do */ 
                    ok = true; 
            }
            else if (mincap_ <= sc_vector_max_cap_(out_vec)) {
                    /* not too big */ 
                    size_t newsize = sc_vector_growsize_((out_vec)->cap);
                    newsize = sc_vector_clamp_(newsize, mincap_, sc_vector_max_cap_(out_vec));
                    void* p = sc_vector_reallocdata_((out_vec)->data, newsize, sizeof(*(out_vec)->data),& (out_vec)->cap, &(out_vec)->size);
                        if (p) {
                                (out_vec)->data = (sc_adb_device*)p;
                        } 
                        ok = (bool)p;
            }
            else {
        ...
        if (ok) {
            ...
            (out_vec)->data[(out_vec)->size++] = (device); 
        }
        //==========================================================
