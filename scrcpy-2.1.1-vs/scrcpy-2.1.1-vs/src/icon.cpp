...
decode_image(const char *path) {
    ...
    stream = av_find_best_stream(ctx, AVMEDIA_TYPE_VIDEO, -1, -1, NULL, 0);
    if (stream < 0 ) {
        LOGE("Could not find best image stream");
        goto close_input;
    }

    params = ctx->streams[stream]->codecpar;
...
load_from_path(const char *path) {
    ...
    format = to_sdl_pixel_format(AVPixelFormat(frame->format));
...
scrcpy_icon_destroy(SDL_Surface *icon) {
    AVFrame *frame = (AVFrame*)icon->userdata;
    assert(frame);
    av_frame_free(&frame);
