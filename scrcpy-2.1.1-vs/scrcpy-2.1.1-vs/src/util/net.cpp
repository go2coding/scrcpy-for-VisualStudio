...
wrap(sc_raw_socket sock) {
    ...
    struct sc_socket_windows *socket = (struct sc_socket_windows*)malloc(sizeof(*socket));
    if (!socket) {
        LOG_OOM();
        closesocket(sock);
        return SC_SOCKET_NONE;
...
net_listen(sc_socket server_socket, uint32_t addr, uint16_t port, int backlog) {
    ...
    int reuse = 1;
    if (setsockopt(raw_sock, SOL_SOCKET, SO_REUSEADDR, (const char *) &reuse,
                   sizeof(reuse)) == -1) {
...
net_send(sc_socket socket, const void *buf, size_t len) {
    sc_raw_socket raw_sock = unwrap(socket);
    return send(raw_sock, (char*)buf, len, 0);
}
