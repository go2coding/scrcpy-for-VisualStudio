...
convert_keycode(enum sc_keycode from, enum android_keycode *to, uint16_t mod,
    ...
    if (entry) {
        *to = android_keycode(entry->value);
        return true;
    }
...
autocomplete_metastate(enum android_metastate metastate) {
    ...
    if (metastate & (AMETA_SHIFT_LEFT_ON | AMETA_SHIFT_RIGHT_ON)) {
        metastate = android_metastate(metastate | AMETA_SHIFT_ON);
    }
    if (metastate & (AMETA_CTRL_LEFT_ON | AMETA_CTRL_RIGHT_ON)) {
        metastate = android_metastate(metastate | AMETA_CTRL_ON);
    }
    if (metastate & (AMETA_ALT_LEFT_ON | AMETA_ALT_RIGHT_ON)) {
        metastate = android_metastate(metastate | AMETA_ALT_ON);
    }
    if (metastate & (AMETA_META_LEFT_ON | AMETA_META_RIGHT_ON)) {
        metastate = android_metastate(metastate | AMETA_META_ON);
    }
...
convert_meta_state(uint16_t mod) {
    enum android_metastate metastate = AMETA_NONE;
    if (mod & SC_MOD_LSHIFT) {
        metastate = android_metastate(metastate | AMETA_SHIFT_LEFT_ON);
    }
    if (mod & SC_MOD_RSHIFT) {
        metastate = android_metastate(metastate | AMETA_SHIFT_RIGHT_ON);
    }
    if (mod & SC_MOD_LCTRL) {
        metastate = android_metastate(metastate | AMETA_CTRL_LEFT_ON);
    }
    if (mod & SC_MOD_RCTRL) {
        metastate = android_metastate(metastate | AMETA_CTRL_RIGHT_ON);
    }
    if (mod & SC_MOD_LALT) {
        metastate = android_metastate(metastate | AMETA_ALT_LEFT_ON);
    }
    if (mod & SC_MOD_RALT) {
        metastate = android_metastate(metastate | AMETA_ALT_RIGHT_ON);
    }
    if (mod & SC_MOD_LGUI) { // Windows key
        metastate = android_metastate(metastate | AMETA_META_LEFT_ON);
    }
    if (mod & SC_MOD_RGUI) { // Windows key
        metastate = android_metastate(metastate | AMETA_META_RIGHT_ON);
    }
    if (mod & SC_MOD_NUM) {
        metastate = android_metastate(metastate | AMETA_NUM_LOCK_ON);
    }
    if (mod & SC_MOD_CAPS) {
        metastate = android_metastate(metastate | AMETA_CAPS_LOCK_ON);
    }
